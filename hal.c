...
uint8_t read_reg8(VM* vm, REG reg_index){
    if (reg_index < 4) {
        ...
        return (uint8_t)(vm->reg[reg_index] & 0x00ff);
    } 
    else {
        // High bits (AH, CH, DH, BH)
        return (uint8_t)(vm->reg[reg_index - 4] >> 8);
    }
}
void write_reg8(VM* vm, REG reg_index, uint8_t value){
     if (reg_index < 4){
        ...
        // high bits
        vm->reg[reg_index - 4] = (vm->reg[reg_index - 4] & 0x00ff) | (value << 8);
    }
}
...
// add seg_reg access
// add current_opcode access
// add ip access
// add flags access
// add is_running access
